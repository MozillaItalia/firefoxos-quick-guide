...
* **Privilegiato:** questo tipo di applicazioni ha accesso a tutte le API disponibili nel browser Firefox, oltre a quelli aggiuntivi, come i contatti, e gli allarmi di sistema. Solo **le app packaged possono essere privilegiate** ed il pacchetto deve essere firmato digitalmente dal Marketplace Firefox.
...
Durante lo sviluppo è possibile accedere alle API privilegiate senza richiedere permessi speciali a Mozilla. In fase di distribuzione però, sarà necessario caricare l'app sul Marketplace Firefox. A questo punto, il codice viene controllato in un rigoroso processo di approvazione e se è tutto OK sarà firmata digitalmente - questo garantirà agli utenti di Firefox OS che a questa applicazione è consentito accedere alle API sensibili.
...
Analizzeremo alcuni esempi di codice per vedere come le API sono facili da utilizzare. Non prendere questi esempi come una documentazione completa sulle WebAPI, sono un piccolo esempio di come si possa accedere al dispositivo usando JavaScript.
